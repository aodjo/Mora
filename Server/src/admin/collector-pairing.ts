...
export async function startCollectorPairing(env: WorkerEnv, value: Record<string, unknown>): Promise<Response> {
  const name = typeof value.name === "string" && value.name.trim().length > 0 && value.name.length <= 100 ? value.name.trim() : "Collector";
  const now = Date.now();
  await env.ADMIN_DB.prepare("DELETE FROM collector_pairings WHERE expires_at<=?1").bind(now).run();
  const active = await env.ADMIN_DB.prepare("SELECT COUNT(*) count FROM collector_pairings WHERE status='pending' AND expires_at>?1")
    .bind(now)
    .first<{ count: number }>();
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    const collision = await env.ADMIN_DB.prepare("SELECT id FROM collector_pairings WHERE pin_hash=?1")
  ...
  await env.ADMIN_DB.prepare(
    "INSERT INTO collector_pairings (id,pin_hash,device_hash,name,expires_at,created_at) VALUES (?1,?2,?3,?4,?5,?6)",
  )
    .bind(id, await sha256(pin), await sha256(deviceCode), name, expiresAt, now)
    .run();
...
export async function pollCollectorPairing(request: Request, env: WorkerEnv, pairingId: string): Promise<Response> {
  ...
  const row = await env.ADMIN_DB.prepare(
    "SELECT status,credential_ciphertext,expires_at FROM collector_pairings WHERE id=?1 AND device_hash=?2",
  )
    .bind(pairingId, await sha256(deviceCode))
    .first<{ status: string; credential_ciphertext: string | null; expires_at: number }>();
  ...
  await env.ADMIN_DB.prepare("UPDATE collector_pairings SET consumed_at=COALESCE(consumed_at,?1) WHERE id=?2")
...
export async function approveCollectorPairing(env: WorkerEnv, actor: Actor, value: Record<string, unknown>): Promise<Response> {
  ...
  await env.ADMIN_DB.batch([
    env.ADMIN_DB.prepare("INSERT INTO service_keys (id,name,prefix,secret_hash,scopes,created_at) VALUES (?1,?2,?3,?4,?5,?6)").bind(
      ...
      secret.slice(0, 13),
      await sha256(secret),
      JSON.stringify(collectorScopes),
      now,
    ),
    env.ADMIN_DB.prepare(
      "UPDATE collector_pairings SET status='approved',credential_ciphertext=?1,approved_by=?2,approved_at=?3 WHERE id=?4 AND status='pending'",
