...
export async function bootstrapOptions(request: Request, env: WorkerEnv, body: Record<string, unknown>): Promise<unknown> {
  ...
  `).bind(challengeId, options.challenge, Date.now() + 5 * 60_000, userId, body.email.toLowerCase(), body.display_name).run();
  return { challenge_id: challengeId, options };
}

export async function bootstrapVerify(request: Request, env: WorkerEnv, body: Record<string, unknown>): Promise<{ user_id: string; cookie: string }> {
  ...
  const relyingParty = await rp(request, env);
  const verification = await verifyRegistrationResponse({ response: body.response as Parameters<typeof verifyRegistrationResponse>[0]["response"], expectedChallenge: challenge.challenge, expectedOrigin: relyingParty.origin, expectedRPID: relyingParty.id, requireUserVerification: true });
  ...
  await env.ADMIN_DB.batch([
    ...
    env.ADMIN_DB.prepare("INSERT OR IGNORE INTO roles (id, name, permissions, system, created_at) VALUES ('owner', 'Owner', '[\"*\"]', 1, ?1)").bind(now),
    env.ADMIN_DB.prepare("INSERT INTO user_roles (user_id, role_id) VALUES (?1, 'owner')").bind(challenge.pending_user_id),
    env.ADMIN_DB.prepare("INSERT INTO webauthn_credentials (id, user_id, public_key, counter, transports, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6)").bind(credential.id, challenge.pending_user_id, credential.publicKey, credential.counter, JSON.stringify(credential.transports ?? []), now),
...
export async function loginOptions(request: Request, env: WorkerEnv, body: Record<string, unknown>): Promise<unknown> {
  ...
  const user = await env.ADMIN_DB.prepare("SELECT id FROM users WHERE email = ?1 AND status = 'active'").bind(body.email.toLowerCase()).first<{ id: string }>();
  if (user === null) throw new ServiceError(401, "UNAUTHORIZED");
  const credentials = await env.ADMIN_DB.prepare("SELECT id, transports FROM webauthn_credentials WHERE user_id = ?1").bind(user.id).all<{ id: string; transports: string }>();
  const relyingParty = await rp(request, env);
  const options = await generateAuthenticationOptions({ rpID: relyingParty.id, userVerification: "required", allowCredentials: credentials.results.map((credential) => ({ id: credential.id, transports: JSON.parse(credential.transports) as AuthenticatorTransport[] })) });
  const challengeId = crypto.randomUUID();
  await env.ADMIN_DB.prepare("INSERT INTO auth_challenges (id, kind, user_id, challenge, expires_at) VALUES (?1, 'authentication', ?2, ?3, ?4)").bind(challengeId, user.id, options.challenge, Date.now() + 5 * 60_000).run();
  return { challenge_id: challengeId, options };
}

export async function loginVerify(request: Request, env: WorkerEnv, body: Record<string, unknown>): Promise<{ user_id: string; cookie: string }> {
  if (typeof body.challenge_id !== "string" || typeof body.response !== "object" || body.response === null) throw new ServiceError(400, "INVALID_REQUEST");
  const response = body.response as { id?: unknown };
  if (typeof response.id !== "string") throw new ServiceError(400, "INVALID_REQUEST");
  const challenge = await env.ADMIN_DB.prepare("SELECT user_id, challenge FROM auth_challenges WHERE id = ?1 AND kind = 'authentication' AND expires_at > ?2").bind(body.challenge_id, Date.now()).first<{ user_id: string; challenge: string }>();
  const credential = await env.ADMIN_DB.prepare("SELECT id, public_key, counter, transports FROM webauthn_credentials WHERE id = ?1").bind(response.id).first<{ id: string; public_key: ArrayBuffer | number[]; counter: number; transports: string }>();
  if (challenge === null || credential === null) throw new ServiceError(401, "UNAUTHORIZED");
  const relyingParty = await rp(request, env);
  const verification = await verifyAuthenticationResponse({ response: body.response as Parameters<typeof verifyAuthenticationResponse>[0]["response"], expectedChallenge: challenge.challenge, expectedOrigin: relyingParty.origin, expectedRPID: relyingParty.id, credential: { id: credential.id, publicKey: bytes(credential.public_key), counter: credential.counter, transports: JSON.parse(credential.transports) as AuthenticatorTransport[] }, requireUserVerification: true });
  if (!verification.verified) throw new ServiceError(401, "VERIFICATION_FAILED");
  await env.ADMIN_DB.batch([
    env.ADMIN_DB.prepare("UPDATE webauthn_credentials SET counter = ?1, last_used_at = ?2 WHERE id = ?3").bind(verification.authenticationInfo.newCounter, Date.now(), credential.id),
    env.ADMIN_DB.prepare("DELETE FROM auth_challenges WHERE id = ?1").bind(body.challenge_id),
  ]);
...
export async function logout(request: Request, env: WorkerEnv): Promise<string> {
  ...
  if (token !== undefined) await env.ADMIN_DB.prepare("DELETE FROM sessions WHERE id_hash = ?1").bind(await sha256(decodeURIComponent(token))).run();
