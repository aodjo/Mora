...
export async function authenticate(request: Request, env: WorkerEnv): Promise<Actor> {
  ...
  if (bearer !== undefined) {
    ...
    `).bind(hash, Date.now()).first<{ id: string; scopes: string }>();
    if (row !== null) {
  ...
  if (session !== undefined) {
    ...
    if (user !== null) {
      ...
      `).bind(user.id).all<{ permissions: string }>();
...
export async function createSession(database: D1Database, userId: string): Promise<{ token: string; cookie: string }> {
  ...
  const now = Date.now();
  await database.prepare("INSERT INTO sessions (id_hash, user_id, expires_at, created_at, last_seen_at) VALUES (?1, ?2, ?3, ?4, ?4)")
    .bind(await sha256(token), userId, now + 12 * 60 * 60 * 1000, now).run();
  return { token, cookie: `mora_session=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=43200` };
}

export async function audit(env: WorkerEnv, actor: Actor | null, action: string, targetType: string, targetId: string | null, summary: Record<string, unknown> = {}): Promise<void> {
  await env.ADMIN_DB.prepare(`
    INSERT INTO audit_log (actor_type, actor_id, action, target_type, target_id, summary, created_at)
    VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)
  `).bind(actor?.type ?? "system", actor?.id ?? null, action, targetType, targetId, JSON.stringify(summary), Date.now()).run();
}
