...
export function Editor({ candidateId, onPublished }: { candidateId: string; onPublished?: () => void }) {
  ...
  return <div className="editor-layout">
    <section className="editor-panel">
      ...
      <div className="audio-tracks">{playableArtifacts.map(artifact=><div key={artifact.id} className="artifact-row"><span>{trackName(artifact)}</span><audio ref={element=>{if(element===null)audioRefs.current.delete(artifact.id);else audioRefs.current.set(artifact.id,element);}} controls preload="metadata" src={`/admin/api/generator/artifacts/${artifact.id}/content`} onPlay={event=>play(artifact.id,event.currentTarget)} onTimeUpdate={event=>setCurrentMs(event.currentTarget.currentTime*1000)} onSeeked={event=>setCurrentMs(event.currentTarget.currentTime*1000)} onLoadedMetadata={event=>{const loadedDuration=event.currentTarget.duration*1000;setMediaDuration(current=>Math.max(current,loadedDuration));}}/></div>)}</div>
