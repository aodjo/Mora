              ...
              <article key={id} className="job-card">
                <div className="job-card-top">
                  <div className="job-identity">
                    <span className={`state-dot ${stateTone(state)}`} />
                    <div>
                ...
                <div className="job-stage-row">
                  <div>
                    <span>현재 단계</span>
                    <strong>{text(item.current_stage, state === "queued" ? "할당 대기" : "준비 중")}</strong>
                  </div>
                  <b>{Math.round(progress * 100)}%</b>
                </div>
                <div className="job-progress" aria-label={`진행률 ${Math.round(progress * 100)}%`}>
                  <i style={{ width: `${progress * 100}%` }} />
                </div>
                <div className="job-meta">
                  <span>
                    <RotateCcw size={13} />
                    {number(item.attempt_count)}/{number(item.max_attempts, 3)}회
                  </span>
                  <span>
                    <UserRound size={13} />
                    {shortId(item.worker_id)}
                  </span>
                  <span>
                    <Clock3 size={13} />
                ...
                <div className="job-actions">
                  ...
                  {["queued", "claimed", "running", "failed"].includes(state) && item.cancel_requested !== 1 && (
                    <button className="job-cancel" onClick={() => void cancel(id)}>
                      <Ban size={14} />
                      취소
                    </button>
                  )}
