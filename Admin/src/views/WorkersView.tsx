...
export function WorkersView({ items, refresh }: { items: AdminItem[]; refresh: () => void }) {
  ...
  async function setState(id: string, desired: "active" | "paused"): Promise<void> {
    ...
    try {
      ...
      showToast(reason instanceof Error ? reason.message : "워커 상태 변경 실패", { variant: "error" });
    } finally {
      setBusyId(null);
    }
  }
  ...
  if (items.length === 0)
    return (
      <div className="empty-panel">
        <Radio size={20} />
        <strong>연결된 Generator가 없습니다</strong>
        <p>기기 연결에서 PIN을 승인하면 워커 상태를 확인할 수 있습니다.</p>
      </div>
    );
  return (
    <div className="worker-grid">
      {items.map((item) => {
        ...
        return (
          <article key={id} className={`worker-card ${online && ready ? "online" : "offline"}`}>
            <div className="worker-head">
              <div className="worker-name">
                <span className="worker-machine">
                  <ServerCog size={18} />
                </span>
                <div>
                  <strong>{text(item.name, "이름 없는 Generator")}</strong>
                  <code title={id}>{shortId(id)}</code>
                </div>
              </div>
              <span className={`worker-presence ${online && ready ? "online" : "offline"}`}>
                <i />
                {online && ready ? "온라인" : "확인 필요"}
              </span>
            </div>
            <div className="worker-specs">
              <div>
                <Cpu size={15} />
                <span>Backend</span>
                <strong>{text(item.backend)}</strong>
              </div>
              <div>
                <Box size={15} />
                <span>Hardware</span>
                <strong>{text(item.hardware)}</strong>
              </div>
            </div>
            <div className="worker-capabilities">
              {capabilities.length > 0 ? capabilities.map((value) => <span key={value}>{value}</span>) : <span>capability 정보 없음</span>}
            </div>
            <div className="worker-checks">
              {checks.slice(0, 6).map(([name, result]) => (
                <span key={name}>
            ...
            <footer>
              <span className={`state-badge ${stateTone(desired)}`}>{stateLabel(desired)}</span>
              <span>v{text(item.version)}</span>
              <time>{relativeTime(item.last_seen_at)}</time>
            </footer>
            <div className="worker-actions">
              {desired === "active" ? (
                <button disabled={busyId === id} onClick={() => void setState(id, "paused")}>
                  <Pause size={13} />
                  일시정지
                </button>
              ) : (
                <button disabled={busyId === id} onClick={() => void setState(id, "active")}>
                  <Play size={13} />
                  재개
                </button>
              )}
