...
export function SettingsPanel() {
  ...
  async function save(item: RuntimeSetting): Promise<void> {
    ...
    if (value.length === 0) {
      showToast("저장할 값을 입력하세요.", { variant: "error" });
      return;
    }
    setBusyKey(item.key);
    try {
      ...
      showToast(reason instanceof Error ? reason.message : "설정 저장 실패", { variant: "error" });
    } finally {
      setBusyKey(null);
    }
  }
  ...
  async function reset(item: RuntimeSetting): Promise<void> {
    ...
    try {
      ...
      showToast(reason instanceof Error ? reason.message : "설정 초기화 실패", { variant: "error" });
    } finally {
      setBusyKey(null);
    }
  }
    ...
    try {
      await api("/notifications", {
        ...
        body: JSON.stringify({
          ...
          name: "Operations",
          url: discord,
          events: ["job.failed", "worker.offline", "queue.backlog", "canary.regression", "auth.anomaly"],
        }),
      });
      setDiscord("");
      showToast("Discord Webhook을 저장했습니다.");
    } catch (reason) {
      showToast(reason instanceof Error ? reason.message : "Discord Webhook 저장 실패", { variant: "error" });
    }
  }
...
function RolesPanel() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      showToast(reason instanceof Error ? reason.message : "역할 정보를 불러오지 못했습니다.", { variant: "error" });
    }
  }, [showToast]);
  useEffect(() => {
    void load();
  ...
  function edit(role: RoleItem): void {
    setEditingId(role.id);
    setName(role.name);
    setPermissions(parseArray(role.permissions).join(", "));
  }
  function reset(): void {
    setEditingId(null);
    setName("");
  ...
  async function submit(): Promise<void> {
    ...
    if (name.trim().length === 0 || parsed.length === 0) {
      showToast("역할 이름과 권한을 입력하세요.", { variant: "error" });
      return;
    }
    setBusy(true);
    try {
      await api("/roles", {
        method: "POST",
        body: JSON.stringify({ ...(editingId === null ? {} : { id: editingId }), name: name.trim(), permissions: parsed }),
      });
      showToast(editingId === null ? "역할을 만들었습니다." : "역할을 수정했습니다.");
      reset();
      await load();
    } catch (reason) {
      showToast(reason instanceof Error ? reason.message : "역할 저장 실패", { variant: "error" });
    } finally {
      setBusy(false);
    }
  }
  ...
  async function assign(userId: string, roleId: string): Promise<void> {
    try {
      await api(`/users/${encodeURIComponent(userId)}/roles`, { method: "POST", body: JSON.stringify({ role_id: roleId }) });
      showToast("역할을 부여했습니다.");
      await load();
    } catch (reason) {
      showToast(reason instanceof Error ? reason.message : "역할 부여 실패", { variant: "error" });
    }
  }

  async function unassign(userId: string, roleId: string): Promise<void> {
    try {
      await api(`/users/${encodeURIComponent(userId)}/roles/${encodeURIComponent(roleId)}`, { method: "DELETE" });
      showToast("역할을 해제했습니다.");
      await load();
    } catch (reason) {
      showToast(reason instanceof Error ? reason.message : "역할 해제 실패", { variant: "error" });
    }
  }
    ...
    <section className="settings-section">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="section-heading">
          ...
          <div>
            <h2>역할과 권한</h2>
            <p>
              권한은 <code>*</code> 또는 <code>jobs.manage</code> 같은 capability 문자열입니다.
            </p>
          </div>
        </div>
        <button onClick={() => void load()} className="secondary-button">
      ...
      <div className="role-form">
        ...
        <input
          ...
          placeholder="candidates.read, candidates.approve, timing.edit"
          className="form-control"
        />
        <button disabled={busy} onClick={() => void submit()} className="primary-button">
          <Plus size={14} />
          {editingId === null ? "역할 추가" : "역할 수정"}
        </button>
        {editingId !== null && (
          <button disabled={busy} onClick={reset} className="secondary-button">
            취소
          </button>
        )}
      </div>
      ...
      <div className="role-list">
        {roles.map((role) => (
          <article key={role.id} className={`role-card${editingId === role.id ? " editing" : ""}`}>
            <div>
              <strong>{role.name}</strong>
              {role.system === 1 && <span className="role-system">시스템</span>}
            </div>
            <div className="permission-chips">
              {parseArray(role.permissions).map((permission) => (
                <span key={permission}>{permission}</span>
              ))}
            </div>
            <button onClick={() => edit(role)} className="secondary-button">
              수정
            </button>
          </article>
        ))}
      ...
      <div className="section-heading mt-6 mb-3">
        ...
        <div>
          <h2>관리자 계정</h2>
          <p>패스키로 등록된 계정에 역할을 부여합니다.</p>
        </div>
      </div>
      {users.length === 0 ? (
        <p className="security-note">등록된 관리자 계정이 없습니다.</p>
      ) : (
          ...
          {users.map((user) => {
            ...
            return (
              <article key={user.id} className="user-card">
                <div>
                  <strong>{text(user.display_name)}</strong>
                  <code>{text(user.email)}</code>
                </div>
                <div className="permission-chips">
                  {assigned.length === 0 ? (
                    <span className="role-empty">역할 없음</span>
                  ) : (
                    assigned.map((roleId) => (
                      <span key={roleId}>
                        {roles.find((role) => role.id === roleId)?.name ?? roleId}
                        <button onClick={() => void unassign(user.id, roleId)} aria-label="역할 해제">
                          <X size={11} />
                        </button>
                      </span>
                    ))
                ...
                <select
                  ...
                  onChange={(event) => {
                    if (event.target.value.length > 0) void assign(user.id, event.target.value);
                  }}
                  className="form-control"
                >
...
function ServiceKeysPanel() {
  ...
  const load = useCallback(async () => {
    try {
      setKeys((await api<{ items: ServiceKeyItem[] }>("/service-keys")).items);
    } catch (reason) {
      showToast(reason instanceof Error ? reason.message : "서비스 키를 불러오지 못했습니다.", { variant: "error" });
    }
  }, [showToast]);
  useEffect(() => {
    void load();
  ...
  async function create(): Promise<void> {
    const parsed = scopes
      ...
      .filter((value) => value.length > 0);
    if (name.trim().length === 0 || parsed.length === 0) {
      showToast("키 이름과 scope를 입력하세요.", { variant: "error" });
      return;
    }
    setBusy(true);
    try {
      const result = await api<{ secret: string }>("/service-keys", {
        method: "POST",
        body: JSON.stringify({ name: name.trim(), scopes: parsed }),
      });
      ...
      showToast(reason instanceof Error ? reason.message : "서비스 키 발급 실패", { variant: "error" });
    } finally {
      setBusy(false);
    }
  }
  ...
  async function revoke(key: ServiceKeyItem): Promise<void> {
    ...
    setBusy(true);
    try {
      await api(`/service-keys/${encodeURIComponent(key.id)}`, { method: "DELETE" });
      showToast("서비스 키를 폐기했습니다.");
      await load();
    } catch (reason) {
      showToast(reason instanceof Error ? reason.message : "서비스 키 폐기 실패", { variant: "error" });
    } finally {
      setBusy(false);
    }
  }
  ...
  async function enrollment(): Promise<void> {
    ...
    try {
      ...
      showToast(reason instanceof Error ? reason.message : "등록 토큰 발급 실패", { variant: "error" });
    } finally {
      setBusy(false);
    }
  }
    ...
    <section className="settings-section">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="section-heading">
          ...
          <div>
            <h2>서비스 키</h2>
            <p>Collector·Generator는 PIN 연결로 자동 발급됩니다. 여기서는 외부 연동용 키를 직접 만듭니다.</p>
          </div>
        </div>
        <button onClick={() => void load()} className="secondary-button">
          <RefreshCw size={14} />
      ...
      <div className="role-form">
        ...
        <input
          ...
          placeholder="dashboard.read, jobs.read"
          className="form-control"
        />
        <button disabled={busy} onClick={() => void create()} className="primary-button">
          <Plus size={14} />키 발급
        </button>
        <button disabled={busy} onClick={() => void enrollment()} className="secondary-button">
          등록 토큰
        </button>
      </div>
      ...
      {issued !== null && (
        <div className="issued-secret">
          <div>
            ...
            <p>지금 복사하세요. 이 값은 다시 조회할 수 없습니다.</p>
          </div>
          ...
          <div className="issued-actions">
            <button
              onClick={() => {
                void navigator.clipboard
                  ...
                  .catch(() => showToast("복사에 실패했습니다. 직접 선택해 복사하세요.", { variant: "error" }));
              }}
            ...
            <button onClick={() => setIssued(null)} className="secondary-button">
              숨기기
            </button>
          </div>
        </div>
      ...
      {keys.length === 0 ? (
        <p className="security-note">발급된 서비스 키가 없습니다.</p>
      ) : (
        <div className="key-list">
          {keys.map((key) => (
            <article key={key.id} className={`key-card${key.revoked_at === null ? "" : " revoked"}`}>
              <div>
                <strong>{text(key.name)}</strong>
                <code>{text(key.prefix)}…</code>
              </div>
              <div className="permission-chips">
                {parseArray(key.scopes)
                  .slice(0, 6)
                  .map((scope) => (
              ...
              <div className="key-meta">
                <span>{key.revoked_at === null ? `마지막 사용 ${relativeTime(key.last_used_at)}` : "폐기됨"}</span>
                <time>{time(key.created_at)}</time>
              </div>
              {key.revoked_at === null && (
                <button disabled={busy} onClick={() => void revoke(key)} className="key-revoke">
                  <Trash2 size={13} />
                  폐기
                </button>
              )}
...
export function ConnectionsPanel() {
  ...
  async function approve(kind: "collector" | "generator", pin: string): Promise<void> {
    try {
      ...
      if (kind === "collector") setCollectorPin("");
      else setGeneratorPin("");
    ...
    <div className="space-y-5">
      <section className="connection-guide">
        <div>
          ...
          <p>
            <code>npm run collector</code> 또는 <code>npm run generator</code>
          </p>
        </div>
        <i />
        <div>
          <span>02</span>
          <strong>10자리 PIN 입력</strong>
          <p>아래의 해당 서비스 입력란에 붙여 넣습니다.</p>
        </div>
        <i />
        <div>
          <span>03</span>
          <strong>자동 인증 완료</strong>
          <p>자격증명은 실행 중인 Mac에 안전하게 저장됩니다.</p>
        </div>
      </section>
      <div className="grid gap-5 lg:grid-cols-2">
        <PairingCard
          ...
          pin={collectorPin}
          onPin={setCollectorPin}
          onApprove={() => void approve("collector", collectorPin)}
          description="PIN은 10분간 유효합니다. 승인되면 Collector가 서비스 키를 직접 받아 저장합니다."
        />
        <PairingCard
          kind="generator"
          title="Generator 연결"
          ...
          pin={generatorPin}
          onPin={setGeneratorPin}
          onApprove={() => void approve("generator", generatorPin)}
          description="환경 점검을 통과한 Generator만 연결을 요청할 수 있습니다."
        />
      </div>
    </div>
  );
...
function PairingCard({
  ...
  return (
    <section className="settings-section pairing-card">
      <div className="section-heading mb-5">
        ...
        <div>
          <h2>{title}</h2>
          <p>
            <code>{command}</code>에 표시된 PIN을 승인합니다.
          </p>
        </div>
      </div>
      <label className="pairing-label" htmlFor={`${kind}-pairing-pin`}>
        10자리 연결 PIN
      </label>
      <div className="flex gap-2">
        <input
          id={`${kind}-pairing-pin`}
          value={pin}
          onChange={(event) => onPin(event.target.value.replace(/[^\d\s-]/gu, ""))}
          inputMode="numeric"
          maxLength={12}
          placeholder="000 000 0000"
          autoComplete="one-time-code"
          className="form-control pairing-input min-w-0 flex-1"
        />
        <button disabled={digits.length !== 10} onClick={onApprove} className="primary-button">
          연결 승인
        </button>
      </div>
