...
export function Auth({ status, onAuthenticated, theme, onToggleTheme }: { status: AuthStatus; onAuthenticated: () => void; theme: Theme; onToggleTheme: () => void }) {
  ...
  async function run(): Promise<void> {
    ...
    try {
      if (!status.bootstrapped || addingPasskey) {
        const path = status.bootstrapped ? "/auth/credential/options" : "/auth/bootstrap/options";
        const verifyPath = status.bootstrapped ? "/auth/credential/verify" : "/auth/bootstrap/verify";
        const payload = status.bootstrapped ? { email } : { email, display_name: name };
        const headers = { authorization: `Bearer ${bootstrapToken}` };
        const created = await api<{ challenge_id: string; options: Parameters<typeof startRegistration>[0]["optionsJSON"] }>(path, { method: "POST", headers, body: JSON.stringify(payload) });
        const response = await startRegistration({ optionsJSON: created.options });
        await api(verifyPath, { method: "POST", headers, body: JSON.stringify({ challenge_id: created.challenge_id, response }) });
      } else {
      ...
      showToast(status.bootstrapped ? addingPasskey ? "현재 도메인에 새 패스키를 등록했습니다." : "로그인했습니다." : "관리자 계정과 패스키를 등록했습니다.");
      onAuthenticated();
    } catch (reason) {
  ...
  return <main className="auth-shell">
    ...
    <section className="auth-card">
      ...
      <div className="auth-heading"><h1>{status.bootstrapped ? addingPasskey ? "이 도메인에 패스키 추가" : "관리자 로그인" : "관리자 계정 만들기"}</h1><p>{status.bootstrapped ? addingPasskey ? "기존 관리자 계정에 현재 도메인용 패스키를 등록합니다." : "등록한 패스키로 안전하게 로그인하세요." : "첫 번째 관리자와 패스키를 등록합니다."}</p></div>
      ...
      {status.bootstrapped && addingPasskey && <label className="field-label">Bootstrap token<input value={bootstrapToken} onChange={(e) => setBootstrapToken(e.target.value)} type="password" className="form-input" autoComplete="off" /></label>}
      {error && <p className="alert error">{error}</p>}
      {embedded&&<p className="alert warning">이 화면은 임베디드 브라우저입니다. 패스키는 새 Safari 또는 Chrome 탭에서 등록해야 합니다.</p>}
      <button disabled={busy || email.length === 0 || ((!status.bootstrapped || addingPasskey) && bootstrapToken.length === 0)} onClick={() => void run()} className="primary-button auth-submit">{busy ? "확인 중…" : embedded ? "새 브라우저 탭에서 열기" : status.bootstrapped ? addingPasskey ? "새 패스키 등록" : "패스키로 로그인" : "관리자 등록"}</button>
      {status.bootstrapped && <button type="button" disabled={busy} className="auth-mode-button" onClick={() => { setAddingPasskey((current) => !current); setError(""); }}>{addingPasskey ? "로그인으로 돌아가기" : "이 도메인에 패스키 추가"}</button>}
      <p className="auth-footnote">{addingPasskey ? "기존 계정과 패스키는 유지됩니다." : "비밀번호 없이 기기의 패스키로 인증합니다."}</p>
    </section>
  </main>;
}
