...
function segmentJapanese(line: string): Array<{ value: string; start: number; end: number }> {
  ...
  const flush = (end: number): void => {
    if (start < 0) return;
    const raw = points.slice(start, end).join("");
    const value = raw.replace(/^\p{P}+|\p{P}+$/gu, "");
    if (value.length > 0) {
      const trim = Array.from(raw).findIndex((char) => !punctuation.test(char));
      const offset = start + Math.max(0, trim);
      output.push({ value: value.normalize("NFC").toLocaleLowerCase("und"), start: offset, end: offset + Array.from(value).length });
    }
    start = -1;
    previous = "";
  };
  for (let index = 0; index <= points.length; index += 1) {
    ...
    if (char === undefined || /^\s$/u.test(char) || punctuation.test(char)) {
      flush(index);
      continue;
    }
    const current = japaneseClass(char);
    if (start < 0) {
      start = index;
      previous = current;
      continue;
    ...
    const compatible =
      current === "mark" ||
      current === previous ||
      (previous === "latin" && current === "number") ||
      (previous === "number" && current === "latin");
    if (!compatible) {
      flush(index);
      start = index;
    }
...
function segments(line: string, language: string): Array<{ value: string; start: number; end: number }> {
  ...
  for (let index = 0; index <= points.length; index += 1) {
    ...
    if (value.length > 0)
      output.push({
        value: value.normalize("NFC").toLocaleLowerCase("und"),
        start: start + Math.max(0, left),
        end: start + Math.max(0, left) + Array.from(value).length,
      });
    start = -1;
  }
  return output;
    ...
    if (!excluded) {
      for (const segment of segments(lineText, language)) {
        ...
        tokens.push({
          ...
          line: lineIndex,
          length: Array.from(segment.value).length,
          type: typeOf(segment.value),
