...
export interface AlignmentCandidate {
   ...
   * 어느 정렬기가 이 타이밍을 냈나.
   *
   * `sung` 은 노래로 학습한 정렬기, `whisperx` 는 애초에 그 길이 아니었던 글(한국어로 안 쓰였다).
   *
   * `fallback` 은 **노래 정렬기로 가다가 죽어서 물러선 것**이다. 지금 Generator 는 이것을 내지
   * 않는다 — 물러서는 대신 곡을 실패시킨다. 남겨 둔 것은 둘 때문이다: 옛 판으로 도는 워커가
   * 아직 보낼 수 있고, 문(`passesQualityGate`)이 이 이름을 막고 있어서 누가 그 길을 되살리면
   * 적어도 조용히 공개되지는 않는다. 검정치마 EVERYTHING 이 그렇게 공개됐었다.
   */
