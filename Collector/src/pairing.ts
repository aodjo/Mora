...
export async function startCollectorPairing(
  adminUrl: string,
  name: string,
  fetchImpl: typeof fetch = fetch,
): Promise<CollectorPairingStart> {
...
export async function pollCollectorPairing(
  adminUrl: string,
  pairing: CollectorPairingStart,
  fetchImpl: typeof fetch = fetch,
): Promise<string | undefined> {
  const response = await fetchImpl(
    `${adminUrl.replace(/\/$/u, "")}/admin/api/collector/pairings/${encodeURIComponent(pairing.pairing_id)}`,
    {
  ...
  if (value.status !== "approved" || typeof value.api_key !== "string" || value.api_key.length === 0)
    throw new Error("PAIRING_BAD_RESPONSE");
  return value.api_key;
}

export async function waitForCollectorPairing(
  adminUrl: string,
  pairing: CollectorPairingStart,
  fetchImpl: typeof fetch = fetch,
): Promise<string> {
