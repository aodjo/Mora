...
# ── 가사가 어느 글자로 적혔는가 ──────────────────────────────────────────────
#
# 정렬기는 이 판정으로 고른다. 음소 모델은 글자 사전이 고정되어 있어, 한국어 모델에는 라틴
# 글자가 한 자도 없고 영어 모델에는 한글이 없다. 잘못 고르면 그 곡의 낱말은 전부 우회로로
# 떨어진다.

# 米津玄師 「IRIS OUT」의 구성 그대로. 가나·한자를 세지 않던 때는 분모가 라틴 96 자뿐이라
# 라틴 비율이 100% 로 나왔고, 그래서 영어로 넘어가 앵커 밀도가 0.00 이었다.
JAPANESE = "アイリスアウト " * 60 + "の光を見た " * 30 + "IRIS OUT " * 12
check("일본어 가사를 영어로 넘기지 않는다", daemon.written_language(JAPANESE, "ja") == "ja",
      daemon.written_language(JAPANESE, "ja"))
# 라틴이 스무 자 미만이라 우연히 살아나던 경우도 그대로 살아야 한다.
check("라틴이 거의 없는 일본어도 그대로", daemon.written_language("あの日の空を " * 40, "ja") == "ja")

# 실제로 영어로 적힌 한국 곡은 여전히 영어 모델로 가야 한다 — 그러라고 있는 규칙이다.
check("영어로 적힌 시트는 영어로 넘긴다",
      daemon.written_language("I know you want it baby " * 30, "ko") == "en")
check("한글 시트는 한글 그대로", daemon.written_language("사랑은 늘 도망가 " * 30, "ko") == "ko")
# 섞인 시트는 넘기지 않는다. 어느 쪽도 확실하지 않으면 적힌 것을 믿는다.
check("반반이면 적힌 것을 믿는다",
      daemon.written_language("사랑해 baby " * 30, "ko") == "ko")
# 한자만 있는 글을 일본어라 부르지 않는다 — 중국어일 수 있고, 한국어에도 드물게 섞인다.
check("한자만으로는 일본어라 하지 않는다",
      daemon.written_language("한자 漢字 섞인 우리 노래 " * 20, "ko") == "ko")
check("글자가 스무 자도 안 되면 따지지 않는다", daemon.written_language("hi", "ko") == "ko")


# ── 어느 말로 받아쓸 것인가 ──────────────────────────────────────────────────
#
# 정렬기 고르기와 다른 질문이다. 로마자로 적힌 한국어 가사는 라틴으로 정렬해야 맞지만 노래는
# 한국어로 불린다 — 그 시트를 보고 영어로 받아쓰라 하면 곡을 통째로 잃는다.

# songs.json 이 일본어 곡을 한국어로 분류해 두었던 실제 사례.
check("한국어로 분류된 일본어 곡을 글자가 바로잡는다",
      daemon.sung_language("君の人生の中に僕はいる " * 20, "ko") == "ja")
check("일본어로 분류된 한국어 곡도 마찬가지", daemon.sung_language("사랑은 늘 도망가 " * 20, "ja") == "ko")
# 로마자로는 뒤집지 않는다. 로마자 시트는 한국어 노래일 수 있다.
check("로마자로는 뒤집지 않는다",
      daemon.sung_language("sarangeun neul domanga " * 20, "ko") == "ko")
check("영어 곡은 영어 그대로", daemon.sung_language("I know you want it " * 20, "en") == "en")
# 섞여 있으면 적힌 것을 믿는다.
check("반반이면 그대로", daemon.sung_language("사랑해 baby everyday " * 20, "ko") == "ko")
check("짧으면 따지지 않는다", daemon.sung_language("あ", "ko") == "ko")
