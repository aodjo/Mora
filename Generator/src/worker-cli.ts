...
interface WorkerCredentials { admin_url: string; worker_id: string; api_key: string; created_at: number }
...
try {
  ...
  process.stdout.write(`Generator 환경 확인 완료: ${selfTest.backend} · ${selfTest.hardware} · ${passedChecks}/${Object.keys(selfTest.checks).length} 검사 통과\n`);

  const adminUrl = process.env.MORA_ADMIN_URL ?? "https://mora.junx.dev";
  let credentials = await readCredentials(credentialFile);
  if (credentials !== undefined && credentials.admin_url !== adminUrl) throw new Error(`saved Generator credential belongs to ${credentials.admin_url}`);
  ...
  if (credentials === undefined) {
    ...
    const pairing = await startGeneratorPairing(adminUrl, process.env.MORA_WORKER_NAME ?? `Generator on ${hostname()}`, {
      ...
      capabilities: Object.entries(selfTest.checks).filter(([, value]) => value === "passed").map(([key]) => key),
      production_ready: selfTest.production_ready,
      self_test: Object.fromEntries(Object.entries(selfTest.checks).map(([key, value]) => [key, value === "passed" ? "passed" : value === "skipped" ? "skipped" : "failed"])) as Record<string, "passed" | "failed" | "skipped">,
    });
  ...
  if (credentials !== undefined && process.env.MORA_WORKER_ID !== undefined && credentials.worker_id !== process.env.MORA_WORKER_ID) throw new Error("MORA_WORKER_ID does not match the credential file");
...
async function readCredentials(path: string): Promise<WorkerCredentials | undefined> {
  try {
    ...
    if (typeof value.admin_url !== "string" || typeof value.worker_id !== "string" || typeof value.api_key !== "string" || typeof value.created_at !== "number") throw new Error("invalid worker credential file");
...
async function readArtifactPublicKey(): Promise<string> {
  ...
  catch { throw new Error(`artifact public key not found: ${path}`); }
}
