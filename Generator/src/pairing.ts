...
export async function startGeneratorPairing(adminUrl: string, name: string, capabilities: WorkerCapabilities, fetchImpl: typeof fetch = fetch): Promise<GeneratorPairingStart> {
...
export async function pollGeneratorPairing(adminUrl: string, pairing: GeneratorPairingStart, fetchImpl: typeof fetch = fetch): Promise<GeneratorPairingCredentials | undefined> {
  const response = await fetchImpl(`${adminUrl.replace(/\/$/u, "")}/admin/api/generator/pairings/${encodeURIComponent(pairing.pairing_id)}`, {
  ...
  if (value.status !== "approved" || typeof value.worker_id !== "string" || typeof value.api_key !== "string" || value.api_key.length === 0) throw new Error("PAIRING_BAD_RESPONSE");
  return { worker_id: value.worker_id, api_key: value.api_key };
}

export async function waitForGeneratorPairing(adminUrl: string, pairing: GeneratorPairingStart, fetchImpl: typeof fetch = fetch): Promise<GeneratorPairingCredentials> {
