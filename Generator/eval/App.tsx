...
import type { AudioHit, Line, LyricHit, Song, Verdict, Word } from "./api";
import { addSong, clock, editSong, fetchState, getSong, listSongs, startFetch } from "./api";
...
export default function App() {
  ...
  const say = useCallback((text: string) => {
    setToast(text);
    window.setTimeout(() => setToast((now) => (now === text ? "" : now)), 2200);
  }, []);
  ...
  const activeIndex = useMemo(() => {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].at <= nowMs - offset) found = i; else break;
    }
    return found;
  }, [lines, nowMs, offset]);
  ...
  const onPick = useCallback(async ({ lyric, audio: picked }: { lyric?: LyricHit; audio?: AudioHit }) => {
    ...
    try {
      if (finder === "audio" && picked && song) {
        ...
        say("음원 바꿈"); await open(song.id); return;
      }
      if (!lyric) return;
  ...
  return (
    <div className="app">
      ...
      <main className="stage">
          ...
          <div className="head-meta">
            <div className="t">
              <input value={song?.title ?? ""} placeholder="제목" disabled={!song}
                ...
                onBlur={(event) => song && patch({ title: event.target.value }).then(() => say("제목 저장"))} />
            </div>
            <div className="a">
              <input value={song?.artist ?? ""} placeholder="아티스트" disabled={!song}
                onChange={(event) => song && setSong({ ...song, artist: event.target.value })}
                onBlur={(event) => song && patch({ artist: event.target.value }).then(() => say("아티스트 저장"))} />
            </div>
          </div>
          <div className="acts">
            {([["good", "맞음"], ["off", "밀림"], ["wrong", "틀림"]] as const).map(([key, label]) => (
              <button key={key} disabled={!song}
                ...
                onClick={() => patch({ verdict: song?.verdict === key ? null : key })}>{label}</button>
            ))}
        ...
        {song && (
          <div className="tabs">
            {([["listen", "듣기"], ["tap", "낱말 찍기"]] as const).map(([key, label]) => (
        ...
        {!song ? (
          ...
          <div className="tap-wrap">
            ...
            <Tapper
              ...
              onChange={saveWords} onShiftLine={shiftLine}
            />
          </div>
        )}
          ...
          <div className="foot-nudge">
            ...
            {[-500, -100, 100, 500].map((step) => (
              <button key={step} disabled={!song}
                onClick={() => patch({ offset_ms: (song?.offset_ms ?? 0) + step })}>
                {step > 0 ? "+" : "−"}{Math.abs(step) / 1000}초
              </button>
            ))}
            <span className="offv">{offset} ms</span>
            <button disabled={!song} onClick={() => patch({ offset_ms: 0 })}>되돌리기</button>
