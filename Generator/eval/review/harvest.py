...
#: 걸렀는데, 목표는 **다 잡는 것**이다 — 바이브가 닫히면 못 받는 것은 그 거르개에 걸린 곡도 마찬가지다.
#: 받을 때는 줄 시각이 한 줄이라도 있으면 담고, 거르는 것은 학습 자료를 고를 때 한다. `duration` 과
#: `line_count` 가 장부에 있으니 그때 얼마든지 가를 수 있다.
LEAST_SECONDS, MOST_SECONDS, LEAST_LINES = 0, 36000, 1
