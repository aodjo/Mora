...
# **우리 정렬을 안 쓴다.** 우리가 놓은 자리로 재면, 줄이 엉뚱한 데(간주)에 놓였을 때
# 「원본은 큰데 보컬이 조용하다」가 나온다 — 목소리가 사라진 것이 아니라 간주를 잰 것이다.
# 밖에서 온 줄 시각을 쓰면 그 혼동이 없다. 다음 줄이 시작할 때까지를 그 줄의 구간으로 본다.
...
for index, since, until in spans:
    ...
    # 보컬이 원본의 이 비율보다 작으면 첫 단계에서 버려진 것으로 본다. 노래하는 대목의
    # 보컬은 보통 원본의 30~60% 다 — 10% 아래면 거의 안 남은 것이다.
    kept = got["보컬"] / max(got["원본"], 1e-9)
    # 리드와 서브를 합쳐도 보컬보다 훨씬 작으면 둘째 단계에서 흩어진 것이다.
